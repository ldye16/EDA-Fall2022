...
matrix1[4, ] # Shows everything in row 4
matrix1[ , 3] # Shows everything in column 3
matrix1[c(12, 14)] # gives 12th and 14th value
matrix1[c(12:14)] # gives a slice from the 12th to the 14th value 
matrix1[2:4, 1:3] #
...
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # categorized number of rows and columns then named them. 
matrix4
...
e <- c("red", "white", "red", NA) # What type of vector? NA is a placeholder for a cell w/o data
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector?
dataframe1 <- data.frame(d,e,f) #
names(dataframe1) <- c("ID","Color","Passed"); #View(dataframe1) This is how to change col names 

dataframe1[1:2,] # Just shows rows 1 through 2 with all columns
dataframe1[c("ID","Passed")] # shows the columns you want to by their names
dataframe1$ID # use $ to get just one specific column 
...
```{r DataExercise-1a}
High_Temps_F <- c(90,91,72,78,86,81,78,75,73,71)
Low_Temps_F <- c(64,52,47,58,60,53,50,50,50,53)
```
...
```{r DataExercise-1b}
fahrenheit.to.celsius <- function(F) {
  celsius <- (F-32)*(5/9) 
  return(celsius)
}
fahrenheit.to.celsius(High_Temps_F)
High_Temps_C <- fahrenheit.to.celsius(High_Temps_F)
Low_Temps_C <- fahrenheit.to.celsius(Low_Temps_F)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
TenDayForcast <- data.frame(High_Temps_F,Low_Temps_F,High_Temps_C,Low_Temps_C)
```
