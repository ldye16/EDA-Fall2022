...
`==` #when you want to compare your value to another value 
`!=` check in variables are different 
`<`  Check for values less than specific number 
`<=` less than equal too
`>`  Greater than
`>=` Greater than equal too
`&`  If you have multiples conditions to check you use & to tell if both are true
`|`  this slath to tell if one or the other is true. 
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #this code is filtering only the rows that have the element 0 on column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #filtering original data. We want column depth to be equal to 0
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # use the "or" operation
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #best method for when you need to select based on more than just two elements. 
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma is interpreted as a cobination of both
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #I am interested in temperature and want the columns that relate to temperature. These are lakename, sampledate, depth, and temperature_C. Because sampledate, depth, and temperature are all next to each other in the data I can put the first column name, then ":", then the last column name.  
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #I want month right after "daynum". 
...
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #this is a much better way of completing this goal. 
```
