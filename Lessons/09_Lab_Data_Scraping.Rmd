...
#Set the URL
Ex.theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
Ex.gage_data <- read.table(Ex.theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 

#Update the column headers
colnames(Ex.gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
Ex.gage_data <- Ex.gage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(Ex.gage_data,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
...
df_electricity <- data.frame(
  "Names" = the_states,
  "Average retail price (cents/kWh)" = the_price,
  "Net summer capacity (MW)" = the_capacity,
  "Net generation (MWh)" = net_generation,
  "Total retail sales (MWh)" = total_retail
)
```
...
df_electricity <- data.frame(
  ...
  mutate(Year = the_year) #this pipe creates a new column allowing us to know the dates associated with multiple combined dataframes. 
...
df_17.19 <- map(seq(2017,2019), scrape.it) %>% 
  bind_rows()
