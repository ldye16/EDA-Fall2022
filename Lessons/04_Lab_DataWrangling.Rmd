...
NTL.phys.data.TuesdayLake <- filter(NTL.phys.data, lakename %in% "Tuesday Lake")

NTL.phys.data.TuesdayLake.1990.1999 <- filter(NTL.phys.data.TuesdayLake, year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Row numbers can be arbitrary. We are more interested in the data itself, not how it is arranged. 
...
NTL.phys.data.TuesdayLake.July.1990.1999 <- 
  NTL.phys.data %>% 
...
NTL.phys.data.TuesdayLake.Surface.July.1990.1999 <- 
  NTL.phys.data.TuesdayLake.July.1990.1999 %>% 
  filter(depth == 0) %>% 
  filter(!is.na(depth)) %>% 
...
NTL.phys.data.irradiance <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradiance", values_to = "radiation")
