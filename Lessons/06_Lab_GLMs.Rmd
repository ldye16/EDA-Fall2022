...
PM2.5RegressionSP <- 
  ...
  geom_point()+
  geom_text(x = 130, y = 10, label = "Y = 0.38x + 15.64", hjust = 1)
print(PM2.5RegressionSP)
...
O3byALL.regression <- lm(data = EPAnum, PM2.5 ~ Ozone + meanLAT + meanLOG + 
              Month + Year + PM2.5)
summary(O3byALL.regression)

step(O3byALL.regression)
...
Select.Regression <- lm(data = EPAnum, PM2.5 ~ Ozone + meanLAT + meanLOG + Month)

summary(Select.Regression)
```
