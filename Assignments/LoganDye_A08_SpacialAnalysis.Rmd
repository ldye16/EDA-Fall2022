...
> ANSWER: 
> The EPSG code is 4269. This is a projected coordinate reference system associate with the NAD83 datum. 
...
> ANSWER:
> The "dec_long_va" column has the x corrdinate values and the "dec_lad_va" has the y coordinate values. 
...
>ANSWER: 
>The sf dataframe has replaced the "dec_lat_va" and "dec_long_va" columns with the new field, "geometry." 
...
ggplot()+
  ...
  geom_sf(data = NWIS_Gage_sf, color = "red") + 
  ggtitle("NWIS Gage Locations (NE)", subtitle = "Logan Dye")
...
mapview(counties_sf, alpha.regions = 0)+
  mapview(NWIS_Gage_sf_join, zcol = "gage_ht")
...
NWIS.Join.sf.utm <- st_transform(NWIS_Gage_sf_join,32614)
...
Lancaster_shp <- counties.sf.utm %>%
  filter(NAME == "Lancaster")

#23 Select gages within the selected county

Lancaster_Gage_Intersects <- NWIS.Join.sf.utm[Lancaster_shp,]
...
ggplot()+
  geom_sf(data = counties.sf.utm) +
  geom_sf(data = Lancaster_shp, color = "red") +
  geom_sf(data = Lancaster_Gage_Intersects)
